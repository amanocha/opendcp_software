...
csr_sparse parse_csr_sparse(char *fname) {
  ...
  outfile << STATIC << TYPE << " G_indptr[" << ret.shape[0]+1 << "] = {0, ";
  ...
  outfile << STATIC << TYPE << " G_indices[" << ret.size << "] = {";
  for (int i = 0; i < ret.size-1; i++) {
    outfile << ret.indices[i] << ", ";
  }
  outfile << ret.indices[ret.size-1] << "};" << endl;

  outfile << STATIC << TYPE << " G_data[" << ret.size << "] = {";
  for (int i = 0; i < ret.size-1; i++) {
    outfile << ret.data[i] << ", ";
  }
  outfile << ret.data[ret.size-1] << "};" << endl;
...
int main(int argc, char** argv) {
  ...
  srand(SEED);
  outfile << STATIC << TYPE << " M[" << G.shape[0]*G.shape[1] << "] = {";
  int entry;
  for (unsigned int i = 0; i < G.shape[0]; i++) {
    for (unsigned int j = 0; j < G.shape[1]; j++) {
      entry = i*G.shape[1] + j; //(int) ((double) rand() / (RAND_MAX));
      if (i == G.shape[0]-1 && j == G.shape[1]-1) {
        continue;
      }
      outfile << entry << ", ";
    }
  ...
  outfile << TYPE << " result_shape[2] = {" << G.shape[0] << ", " << G.shape[1] << "};" << endl;
  outfile << TYPE << " result_indptr[" << G.shape[0]+1 << "] = {";
  for (int i = 0; i < G.shape[0]; i++) {
    outfile << "0, ";
  }
  outfile << "0};" << endl;

  outfile << TYPE << " result_indices[" << G.size << "] = {";
  for (int i = 0; i < G.size-1; i++) {
    outfile << "0, ";
  }
  outfile << "0};" << endl;

  outfile << TYPE << " result_data[" << G.size << "] = {";
  for (int i = 0; i < G.size-1; i++) {
    outfile << "0, ";
  }
  outfile << "0};" << endl;
