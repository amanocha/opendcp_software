  ...
  for(uint32_t j = id; j < B_ncols; j+=core_num) {
    ...
    for(uint32_t k = B_start; k < B_end; k++) {
      ...
      for(uint32_t m = A_start; m < A_end; m++) {
        ...
        compute_exclusive_store((int*) (spa + j*A_nrows + A_idx), (int) (A * B + data));
    ...
    for(uint32_t i = 0; i < A_nrows; i++) {
      ...
      if(tmp_spa) {
        ...
        if(tmp_spa) {
          ...
          compute_exclusive_store((int*) (tmp_C_indices + index), (int) i);
          tmp_spa = 0;
        }
      }
      compute_exclusive_store((int*) (spa + index), (int) tmp_spa);
