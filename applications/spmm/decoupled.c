  ...
  for(uint32_t j = id; j < B_ncols; j+=core_num) {
    ...
    for(uint32_t k = B_start; k < B_end; k++) {
      ...
      for(uint32_t m = A_start; m < A_end; m++) {
        ...
        uint32_t A = A_data[m];
        uint32_t index = j*A_nrows + A_idx;
        uint32_t res = A * B;
        compute_exclusive_store(spa + index, res);
      }
    }
    ...
    for(uint32_t i = 0; i < A_nrows; i++) {
      ...
      if(tmp_spa) {
        ...
        if(tmp_spa) {
          ...
          compute_exclusive_store(tmp_C_indices + index, i);
          compute_exclusive_store(spa + index, tmp_spa);
        }
      }
    }
